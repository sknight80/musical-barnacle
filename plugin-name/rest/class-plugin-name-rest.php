 ...
 * @author     Istvan Szabo <istvan.szabo@brightx.com>
 */
class Plugin_Name_Rest  {
  ...
  /**
    ...
    $base = 'route';    // ntfs: /wp-json/plugin_name/v1
    	register_rest_route( $namespace, '/' . $this->plugin_name . '/', 
    		array(
	...
	// access method. I'm showing only one here. 
    	if ( ! (current_user_can( 'edit_users' ) || current_user_can('edit_gc_operations') || current_user_can('edit_gc_dues') ||
     	   ...
     	   return new \WP_Error( 'rest_forbidden', esc_html__( 'Sorry, you are not authorized for that.', 'plugin_name' ), array( 'status' => 401 ) );
    	}
